...
@app.get("/api/recipes/<string:name>")
...
@app.delete("/api/recipes/<string:name>")
...
def update_recipe_order():
    ...
    conn = db_conn()
    existing = {
        row["name"] for row in conn.execute("SELECT name FROM recipes WHERE name IN (%s)" % ",".join("?" * len(normalized)), normalized).fetchall()
